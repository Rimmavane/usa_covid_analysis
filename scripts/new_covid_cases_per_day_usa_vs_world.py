...
fig = px.line(usa_data, x="date", y='new_cases', color="location", hover_name="location")

layout = go.Layout(
    title=go.layout.Title(
        text="Daily COVID-19 new cases count in 9 countries with most COVID-19 cases and world",
        x=0.5
    ),
    font=dict(size=14),
    width=1200,
    height=800,
    xaxis_title="Date",
    yaxis_title="Confirmed new cases count per day",
    transition={'duration': 333}
)
fig.update_layout(layout)
fig.update_yaxes(rangemode="nonnegative")
fig.write_html('../plots/new_infection_cases_covid19_worldwide_with_world.html')