...
fig = px.bar(usa_data, x="location", y='total_deaths', animation_frame="date", animation_group='location',
                  color="location", hover_name="location")

layout = go.Layout(
    title=go.layout.Title(
        text="Total COVID-19 deaths count in 9 countries with most COVID-19 cases and world",
        x=0.5
    ),
    font=dict(size=14),
    width=1200,
    height=800,
    xaxis_title="",
    yaxis_title="Total deaths count",
    transition={'duration': 333}
)
fig.update_layout(layout)
fig.update_yaxes(rangemode="nonnegative")
fig.write_html('../plots/total_deaths_covid19_worldwide_with_world.html')